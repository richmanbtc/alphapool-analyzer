...
class DataFetcherBuilder:
    def __init__(self):
        self.bybit_rate_limiter = RateLimiterGroup(
            limits=scale_limits(bybit_limits(), 0.5)
        )
        self.ftx_rate_limiter = RateLimiterGroup(limits=scale_limits(ftx_limits(), 0.5))
        self.binance_rate_limiter = RateLimiterGroup(
            limits=scale_limits(binance_limits(), 0.5)
        )
        self.okex_rate_limiter = RateLimiterGroup(
            limits=scale_limits(okex_limits(), 0.5)
        )
    ...
    def _create_ftx(self):
        ...
        wrap_object(
            client, rate_limiter_group=self.ftx_rate_limiter, wrap_defs=ftx_wrap_defs()
        )
        return client
